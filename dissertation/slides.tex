...
\frame{ \frametitle{Motivation}
  \begin{itemize} 
    ...
    \item Students have variance in \emph{trait ability}, that is internalized
    problem-solving ability, with respect to certain skills.  In a classroom
    this trait ability is normally distributed.
    
    \item Targetting any segment of the distribution of students provides that
    segment with better treatment at the expense of other segments.  Targetting
    the middle neglects the wings (both high-performing and low-performing).
...
\frame{ \frametitle{Motivation}
  ...
  \begin{itemize} 
    ...
    Accommodating all of these with a classical approach would seem to be
    impossible.

    \item Ideally, we would like to personalize teaching; but the amount of
    time required to do so is prohibitive. Thus we introduce ITS (intelligent
    tutoring systems) to help us.
...
\frame{ \frametitle{The Odds Ratio}
  ...
  \[
    logit_j = e^{b_1 p_1 + b_2 p_2 + \ldots b_i (1) \ldots b_n p_n}
  \]
